...
run_simulations_with_noise <- function(ic_list = NULL,
  ...
  out <- lapply(seq_along(p_ic_list), function(ic) {
    ...
    for(i in 2:4) names(noisysimulation[[i]]) <- paste0("r", outer(1:(length(which_pars_perturbed)),1:length(which_pars_perturbed), FUN = paste0))
